...
class FacebookScraper(BaseScraper):
    ...
    def scrape(
        ...
        target_keywords = keywords or ["software development", "odoo partner", "zoho crm"]

        self.logger.info(f"Scraping live Facebook business leads for {target_locations}...")

        if not DDGS:
            self.logger.warning("DDGS module not available. Skipping social query.")
            return leads
                ...
                for loc in target_locations:
                    for kw in target_keywords:
                        if len(leads) >= max_results:
                            break
                        query = f'site:facebook.com "{kw}" "{loc}" email OR contact'
                        try:
                            for res in ddgs.text(query, max_results=5):
                                ...
                                if valid_email:
                                    comp_name = title.split("-")[0].split("|")[0].replace("Facebook", "").strip()
                                    leads.append(create_standard_lead(
                                        company_name=comp_name or f"{kw.title()} Agency",
                            ...
                            self.logger.warning(f"Error querying DDG for Facebook: {e}")
        except Exception as e:
            self.logger.warning(f"Facebook search error: {e}")
